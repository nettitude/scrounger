# -*- coding: utf-8 -*-
#
# Original: https://github.com/inaz2/proxy2
#
# Modified by RDC - 24 September 2018
# Removed prints
# Modified certificate key locations
# Modified to save connections and requests
# Added Async Thread + creatio method
...
def join_with_script_dir(base, path):
...
class ProxyRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "http://proxy2.test/" or "://localhost" in self.path \
        or "://127.0.0.1" in self.path or self.path.endswith("/ca.crt"):
            self.send_cacert()
            return
        ...
        try:
            ...
            # Modified by RDC - send cacert instead
            self.send_error(502)
            #self.send_cacert()
            return
    ...
    def print_info(self, req, req_body, res, res_body):
        def parse_qsl(s):
            return "\n".join("%-20s %s" % (k, v) for k, v in urlparse.parse_qsl(
        ...
        req_header_text = "%s %s %s\n%s" % (req.command,
            req.path, req.request_version, req.headers)
        res_header_text = "%s %d %s\n%s" % (res.response_version,
            res.status, res.reason, res.headers)
...
class AsyncThread(threading.Thread):
    """Represents an intercepting proxy server thread"""
...
def create_server(host="", port=9090, cert_path="."):
    ...
    :param str host: the hostname for the proxy to listen on
    :param int port: the local port to open the proxy on
    :param int cert_path: location of the certificates
